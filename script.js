// script.js
// Persian texts and behavior for the valentine generator

const messages = [
  "هر لحظه با تو، مثل یک قطعهٔ کاملِ لگوست — دقیق و خوشگل ❤️",
  "با تو هر روز، یه ساختنی جدید و دل‌چسبه. ولنتاین مبارک! 🌹",
  "تو همون قطعه‌ای که منو کامل می‌کنه. همیشه کنارت می‌مونم 💫",
  "عشق من، تو مثل لبخندِ یه فیگور لگویی — ساده و دوست‌داشتنی 😘",
...
// spawn floating hearts for confetti-like effect
function spawnHearts(count = 6) {
  for (let i=0;i<count;i++){
    ...
    heart.animate([
      { transform: `translateY(0) scale(${0.7 + Math.random()*0.5}) rotate(0)`, opacity: 1 },
...
function getRandomAccent(){
  const colors = ["#ff3b8a","#ff7aa2","#ff6a6a","#ff5c9a","#ff9db1"];
  return colors[randInt(colors.length)];
}

// button behaviors
newBtn.addEventListener("click", () => {
  // change message + small shuffle of figures (we can flip the order randomly)
  showRandomMessage(true);
  // occasionally swap positions for fun
  if (Math.random() < 0.45) {
    // reverse order briefly
    const parent = figures[0].parentElement;
    parent.insertBefore(figures[1], figures[0]);
    // refresh figures array reference
    setTimeout(()=> parent.insertBefore(figures[0], figures[1]), 800);
  }
});

// surprise button: copy page URL to clipboard (user can share)
